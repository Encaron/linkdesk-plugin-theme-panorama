...
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`theme-panorama`）。当前版本 `1.0.4`。

## 1. 这是什么

整窗主视觉——把 `background` 域拉到极限：一张全窗低遮罩主视觉图，chrome 半透明让位。

**用户在哪看到它**：外观主题：在设置 → 外观里切换。
